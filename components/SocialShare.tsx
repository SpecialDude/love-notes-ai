...
const SocialIcon: React.FC<{
  ...
  onClick: () => void;
  label: string;
}> = ({ path, color, onClick, label }) => (
  <button
    onClick={onClick}
    className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center text-white transition-all hover:scale-110 hover:shadow-lg active:scale-95`}
...
export const SocialShare: React.FC<Props> = ({ url, message = "Check out this LoveNote!", className = "" }) => {
  ...
  return (
    <div className={`flex flex-col gap-4 ${className}`}>
      ...
      {/* Feedback toast for IG copy */}
      {copied && (
         <div className="absolute left-1/2 -translate-x-1/2 -top-12 bg-black/80 text-white px-4 py-2 rounded-full text-sm font-bold flex items-center gap-2 animate-in fade-in zoom-in duration-200">
             <Check size={14} /> Link copied for Instagram!
         </div>
      )}
    </div>
  );
